...
def make_boss_embed(data, msg=""):
    ...
    boss_bar = hp_bar(hp_pct_boss, 12)
    player_bar = hp_bar(hp_pct_player, 10)

    # Cor dinâmica do boss — vermelho sangue mais escuro ao ficar fraco
    if hp_pct_boss > 0.5:
    ...
    # Cabeçalho boss — como o HTML tem boss-name e boss-title
    embed.title = f"💀 BOSS:  {boss.get('e','')}  {boss.get('n','?')}"
    embed.description = f"*{boss.get('title', 'Chefe Lendário')}*"

    # HP do Boss com barra grande — como boss-hpb no HTML
    embed.add_field(
        name=f"🔴 HP do Boss — {boss_hp}/{boss_max}",
        value=f"```\n{boss_bar}\n```",
        inline=False
    )
    ...
    embed.set_footer(text=f"💰 Recompensa: {reward} ouro  ·  🪨 Drops: {mat_names}")
...
class BossView(discord.ui.View):
    """Botões da batalha de Boss — estilo HTML."""
    ...
    async def boss_attack(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        lines.append(f"👹 **{boss['e']} {boss['n']}** atacou! **-{boss_dmg} HP**")
    ...
    @discord.ui.button(label="Defender", emoji="🛡️", style=discord.ButtonStyle.secondary)
    async def boss_defend(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        data["defending"] = True
        boss = data["boss"]
        boss_dmg = max(1, int(boss["atk"] * random.uniform(0.3, 0.6)))
        data["playerHp"] = max(0, data.get("playerHp", 100) - boss_dmg)

        msg = f"🛡️ Defendeste! Dano reduzido para **{boss_dmg}**\n❤️ HP: **{data['playerHp']}/{data.get('playerMaxHp',100)}**"
    ...
    @discord.ui.button(label="Tentar Capturar", emoji="🔮", style=discord.ButtonStyle.primary)
    async def boss_catch(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if random.random() < catch_chance:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"# 🎊 Incrível!\n"
                    f"O boss foi adicionado à tua equipa/box!\n\n"
            ...
            embed = make_boss_embed(
                data,
                f"🌀 Ball falhou! (**{int(catch_chance*100)}%** de chance)\n🔮 Balls: **{data.get('balls',0)}**"
            )
            await interaction.response.edit_message(embed=embed, view=self)
...
async def pokedex(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"📖 Pokédex — {len(caught)}/{len(MONS)} monstros",
        description=f"**{pct}%** completado · 👹 {len(bosses)}/{len(BOSSES)} bosses derrotados",
        color=0xffd700
    )

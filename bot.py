...
SHOP_ITEMS = [
    ...
    {"id":"neoncage",   "n":"Gaiola Néon",      "e":"🟩","desc":"+35% captura em Néon/Mecânico/Nuclear","price":160},
...
def get_catch_chance(wild_mon, player_data, using_golden=False):
    ...
    # Gaiola Néon: +35% captura em Néon, Mecânico e Nuclear (igual ao HTML)
    neoncage_bonus = 0.0
    if (player_data.get("items", {}).get("neoncage", 0) > 0
        and wild_mon.get("t", "") in ("néon", "mecânico", "nuclear")):
        neoncage_bonus = 0.35
    total = min(0.97, base + battle_bonus + catch_bonus + neoncage_bonus - penalty)
    if using_golden:
        ...
        if random.random() < catch_chance:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"❤️ **{data['bossMaxHp']:,}** HP · ⚔️ **{boss['atk']:,}** ATK\n"
        ...
        if data["nicoPotions"] >= 3:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "Usa `/boss` para enfrentá-la agora!"
                ),
                color=0xff00ff
            )
...
async def pokedex(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            + ("🌟 **POKÉDEX COMPLETA!** Usa `/desafiar_final` para enfrentar o DEUS ABSOLUTO!" if complete and not final_done else "")
...
async def boss_cmd(interaction: discord.Interaction, nome: Optional[str] = None):
    ...
    # Verifica pendingBoss (ex.: Nico ativado via 3 poções no OXIGÉNIO)
    pending = data.get("pendingBoss")
    if pending and not nome:
    ...
    if nome:
        ...
        # Bloqueia acesso direto ao boss final
        if boss.get("special") == "final_boss":
        ...
        # Bloqueia Nico direto (só via evento secreto)
        if boss.get("special") == "nico" and pending != "Nico":
            await interaction.response.send_message(
                "🐈 **Nico** é um **boss secreto**! Usa 3 **Poções** no monstro **OXIGÉNIO** para o invocar.",
                ephemeral=True
            ); return
    elif not pending or not BOSS_INDEX.get(pending):
        defeated = data.get("bossDefeated", [])
        available = [b for b in BOSSES if b["n"] not in defeated and b.get("special") not in ["master_only", "nico", "final_boss", "murilo"]]
        if not available:
            available = [b for b in BOSSES if b.get("special") not in ["master_only", "nico", "final_boss", "murilo"]]
        boss = random.choice(available)

    _start_boss_battle(data, boss, mon)
    write_save(uid, data)

    embed = make_boss_start_embed(boss, mon, data)
    view = BossView(uid)
    await interaction.response.send_message(embed=embed, view=view)


@tree.command(name="desafiar_final", description="🌌 Desafia o DEUS ABSOLUTO (requer Pokédex completa!)")
async def challenge_final(interaction: discord.Interaction):
    uid = interaction.user.id
    data = load_save(uid)
    ...
    if not is_pokedex_complete(data):
        await interaction.response.send_message(
            f"❌ A tua Pokédex ainda não está completa!\n"
            f"Progresso: **{pokedex_progress(data)}/{pokedex_total()}**\n\n"
            "💡 Captura todos os monstros e derrota todos os bosses para desbloquear o **DEUS ABSOLUTO**!",
            ephemeral=True
        ); return

    if "???" in data.get("bossDefeated", []) or "Leonking" in data.get("bossDefeated", []):
        await interaction.response.send_message("👑 Já derrotaste o DEUS ABSOLUTO LEONKING!", ephemeral=True); return
    ...
    if not final:
        await interaction.response.send_message("❌ Erro: Boss final não encontrado!", ephemeral=True); return
...
async def bosses_list(interaction: discord.Interaction):
    ...
    for boss in BOSSES:
        ...
        special_tag = ""
        if boss.get("special") == "master_only":
            special_tag = " 👑 *Master Ball only*"
        elif boss.get("special") == "nico":
            special_tag = " 🐈 *Boss Secreto (3x Poção em OXIGÉNIO)*"
        elif boss.get("special") == "murilo":
            special_tag = " 👨‍🦽 *Boss Secreto*"
        elif boss.get("special") == "final_boss":
            if complete and boss["n"] not in defeated:
                special_tag = " 🌌 **BOSS FINAL — DESBLOQUEADO!**"
            else:
                special_tag = f" 🌌 *Boss Final — Requer Pokédex Completa ({pokedex_progress(data)}/{pokedex_total()})*"

        # Oculta stats do boss final se ainda está bloqueado
        if boss.get("special") == "final_boss" and not complete:
            value = f"*???*\n**???** · ❤️??? · ⚔️??? · 💰???"
        else:
            value = (
                f"*{boss.get('title','?')}*\n"
                f"{type_badge(boss.get('t','?'))} · ❤️{boss['hp']:,} · ⚔️{boss['atk']:,} · 💰{boss['reward']:,}"
            )
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="🤫 Segredos",
        value=(
            "🐈 **Nico** — Usa 3 **Poções** no monstro **OXIGÉNIO** para o invocar!\n"
            "👑 **Void King** — Só capturável com **Master Ball**\n"
            "🌌 **Boss Final (???)** — Pokédex completa → `/desafiar_final`\n"
